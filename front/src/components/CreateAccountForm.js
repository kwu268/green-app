  ...
  return (
    <div className="flex flex-col  items-center  w-full h-5/6 ">
      <div className=" text-center h-1/3  flex flex-col  w-4/5 "> 
        ...
        <p className=" text-gray-500">Welcome to Greeen!</p>
      </div>
      <form
        className="flex flex-col w-4/5  gap-3"
        onSubmit={submitNewUser}
      >
        <input className="py-1 rounded-md" type="text" placeholder={`Email`} />
        <input className="py-1 rounded-md" type="password" placeholder={`Password`} />
        <input className="py-1 rounded-md" type="text" placeholder={`Display Name`} />
        <button className=" px-4 py-2 text-stone-200 font-medium rounded-md w-full shadow-xl bg-emerald-700 hover:bg-emerald-400" type="submit">Create account</button>
      </form>
    </div>
  );
}
